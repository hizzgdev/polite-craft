...
2. 选择语气（建设性/鼓励/直接/正式）和对方关系（贡献者/同事/熟人/上级）
3. 点击 "Generate English" → 扩展自动抓取页面上下文，调用 DeepSeek API 生成地道英文，并替换输入框内容

零复制粘贴。

## 安装（不通过 Chrome Web Store）
...
| 元素 | 说明 |
|------|------|
| 语气下拉 | 建设性建议 / 温和鼓励 / 直接反馈 / 正式汇报 |
| 关系下拉 | 社区贡献者 / 同事 / 熟人/长期合作 / 上级/负责人 |
| Generate English 按钮 | 点击后自动抓取上下文 → 调用 AI → 替换输入框内容 |
